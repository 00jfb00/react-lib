  ...
  methods: {
    ...
    click(event) {
      ...
      if (parent && parent.className.match(/(rsg--item)/)) {
        ...
        parent.parentNode.classList.add("ul-active-block")
      }
    },
    init() {
      let currentURL = ""
      ...
      if (sidebar) {
        ...
        if (!currentURL.includes('Componentes')) {
          const ulElements = document.querySelectorAll(".ul-active-block");
          if (ulElements.length > 0) {
            ;[].forEach.call(ulElements, function(element) {
              element.classList.remove("ul-active-block")
            })
          }
        }
